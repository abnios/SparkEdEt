...
import { _Units,_Topics,_Resources,_SearchData, _Statistics ,_Deleted,_Egranary,_Settings } from '../../Collections/collections.js';
...
const mCollections = {unit:_Units,topic:_Topics,resource:_Resources,'delete':_Deleted,'egranary':_Egranary};
...
Meteor.methods({
  sendStatistics:function(statistics){
    ...
    HTTP.call("POST", url,
          {params:{statistics:statistics,sch:school}},
...
Meteor.methods({
  getResourceProgress: function(resourcesData) {
    ...
    return{resourceName:resourceName,resourceSize:resourceSize,
      chunkSizeDiff:chunkSizeDiff,resourceIndex:resourceIndex,
      chunkSize:chunkSize,resources:resources.length,resourceIndex:resourceIndex};
...
//  })
  ...
  Meteor.methods({
    runUpgrade :function(sudoPass,version){
      //sh manoap.lite.sh
      const child = exec(' bash ~/manoap.lite.sh', {name:"Upgrade"
      }, (error, stdout, stderr) => {
